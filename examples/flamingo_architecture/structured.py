...
# --- Inter-core topology ---
# Deviation from the original script: the paper's two 399-qubit topologies were
# selected by commenting/uncommenting the three blocks below. They are now
# chosen by this one switch. See examples/README.md.
TOPOLOGY = os.environ.get("DQC_TOPOLOGY", "a2a")   # "a2a" | "line"
if TOPOLOGY not in ("a2a", "line"):
    raise ValueError(f"TOPOLOGY must be 'a2a' or 'line', got {TOPOLOGY!r}")
...
# ==========================================
# 1. HARDWARE & TOPOLOGY SETUP (HERON)
# ==========================================
n_qpus = 3
...
try:
    ...
    print("Could not connect to IBM Runtime Service. Using local FakeTorino backend instead.")
    backend = FakeTorino()
m_n_qbts = backend.num_qubits  # 133
TOTAL_QUBITS = m_n_qbts * n_qpus

# Define inter-QPU edges (Flamingo Architecture)
raw_links = [
    # QPU 0 <-> QPU 1
    (129, 265), (130, 264),
    # QPU 1 <-> QPU 2
...
if TOPOLOGY == "a2a":
    # QPU 0 <-> QPU 2: present only in the all-to-all topology.
    raw_links += [(131, 396), (132, 395)]
inter_qpu_edges = raw_links + [(v, u) for u, v in raw_links]

mqpu_backend = generate_multi_qpu_backend_from_monolithic_backend_with_links(
    n_qpus=n_qpus,
    monolithic_backend=backend,
    inter_qpu_links=inter_qpu_edges,
    backend_name=f'{n_qpus}_Heron_r1'
)

# Cost matrix for KaHyPar layout matching
if TOPOLOGY == "line":
    core_cost_matrix = {
        0: {0: 0.0, 1: 1.0, 2: 2.0},
        1: {0: 1.0, 1: 0.0, 2: 1.0},
...
server_qubits = {
    0: range(0, m_n_qbts), 
    1: range(m_n_qbts, 2 * m_n_qbts), 
    2: range(2 * m_n_qbts, 3 * m_n_qbts)
}

qubit_qpu_map = [i // m_n_qbts for i in range(mqpu_backend.num_qubits)]
if TOPOLOGY == "line":
    inter_qpu_coupling_map = [(0, 1), (1, 0), (1, 2), (2, 1)]
else:
...
# Renamed to S_matrix_1_10 to match your other file
...
def run_kahypar_qig_experiment(qc, seed, seed_start_time, timeout_limit, f_weight=10, S_matrix=S_matrix_default, extended_set_length=20, cla=False, name="KaHyPar QIG + Default SABRE"):
    ...
    mapping = get_heterogeneous_core_assignment(local_qc, core_cost_matrix, KAHYPAR_CONFIG_PATH, max_capacity=m_n_qbts)
...
def run_single_seed_task(
    ...
    penalized_swaps=None, qubit_qpu_map=None, inter_qpu_coupling_map=None, 
...
if __name__ == "__main__":
    ...
    large_dir = os.path.join(BENCHMARKS_DIR, 'large')
    CIRCUITS_TO_RUN = [os.path.join(large_dir, f) for f in os.listdir(large_dir) if f.endswith('.qasm')]
    ...
    CIRCUITS_TO_RUN = BENCHPRESS_TO_RUN + CIRCUITS_TO_RUN # Do Hamiltonian circuits first, then large benchmarks from the other dir. 

    for circuit in CIRCUITS_TO_RUN:
        print(f"Planned circuit for experiment: {circuit.split('/')[-1]}")

    # Do 'hhl', 'qaoa', and 'quantum_volume' last since they take longer
    CIRCUITS_TO_RUN = [c for c in CIRCUITS_TO_RUN if not any(x in c for x in ['hhl', 'qaoa', 'quantum_volume'])] + \
                      [c for c in CIRCUITS_TO_RUN if any(x in c for x in ['hhl', 'qaoa', 'quantum_volume'])]

    TARGET_SUCCESSFUL_SEEDS = 5  
    STARTING_SEED = 42           
    PER_SEED_TIMEOUT = 3 * TARGET_SUCCESSFUL_SEEDS * 3600 # seconds (adjust as needed based on circuit size and expected runtime)

    RESULTS_FILE = os.path.join(RESULTS_DIR, f"flamingo_{TOPOLOGY}_structured.json")
    db = load_experiment_database(RESULTS_FILE)
    WORKERS = 2 # Drop to 2 for very large circuits to avoid RAM issues
    ...
    # Updated indexing to reflect all 8 methods!
    DQC1_IDX = get_or_create_method_index(db, "KaHyPar QIG + Default SABRE", "#cb8331")
    DQC2_IDX = get_or_create_method_index(db, "KaHyPar QIG + (1,10) SABRE", "#cb8331")
    DQC3_IDX = get_or_create_method_index(db, "KaHyPar QIG + Custom Lookahead SABRE", "#cb8331")
    DQC4_IDX = get_or_create_method_index(db, "KaHyPar QIG + Custom Lookahead SABRE (e=100)", "#cb8331")
    ...
    for file_path in CIRCUITS_TO_RUN:
        ...
        if 'rg_qft_multiplier' in circuit_name:
            print(f"Skipping {circuit_name} due to extreme size and known outlier status.")
            continue
