...
try:
    ...
    backend = FakeTorino()
    # backend.refresh(service) # Since we don't care about noise, this isn't strictly necessary, but can be uncommented if you want to pull the latest backend snapshots!
...
def _generate_base_circuit(cz_frac, seed):
    ...
    qc = build_cz_fraction_circuit(n=TOTAL_QUBITS, d=TOTAL_QUBITS, p=cz_frac, seed=seed)
...
def run_kahypar_qig_experiment(qc, orig_intra_swaps, orig_inter_swaps, seed, seed_start_time, timeout_limit, f_weight=10, S_matrix=S_matrix_default, extended_set_length=20, cla=False, name="KaHyPar QIG + Default SABRE"):
    ...
    mapping = get_heterogeneous_core_assignment(qc, core_cost_matrix, KAHYPAR_CONFIG_PATH, max_capacity=m_n_qbts)
        ...
        if cla: # Custom Lookahead enabled
            pm = PassManager([
                ...
                SabreLayout(
                    ...
                    alpha=9.0,
                    beta=3.0,
                    extended_set_length=extended_set_length, # Hardcoded for now, can be tuned
...
if __name__ == "__main__":
    ...
    PER_SEED_TIMEOUT = 6 * TARGET_SUCCESSFUL_SEEDS * 3600 # 30 hours per seed, can be adjusted based on expected runtimes and resource availability. 

    RESULTS_FILE = os.path.join(RESULTS_DIR, f"flamingo_{TOPOLOGY}_cz_frac.json")
    db = load_experiment_database(RESULTS_FILE)
    
    WORKERS = 2 # Drop to 2 for large/dense circuits if needed
    ...
    DQC1_IDX = get_or_create_method_index(db, DQC_LABEL_1, "#cb8331") # Placeholder colours.
    DQC2_IDX = get_or_create_method_index(db, DQC_LABEL_2, "#cb8331")
    DQC3_IDX = get_or_create_method_index(db, DQC_LABEL_3, "#cb8331")
    DQC4_IDX = get_or_create_method_index(db, DQC_LABEL_4, "#cb8331")
